...
const employeeNav = [
    ...
    { name: "EOD Reports", path: "/employee/reports", icon: "📝" },
];
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-gray-950">
            ...
            <aside className={`fixed top-0 left-0 z-50 h-full w-[260px] border-r border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900 transition-transform lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
                <div className="flex h-16 items-center gap-3 border-b border-gray-100 px-6 dark:border-gray-800">
                    <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-to-br from-violet-500 to-purple-600 text-white text-sm font-bold">E</div>
                    <div>
                        <h2 className="text-sm font-bold text-gray-900 dark:text-white">Employee Portal</h2>
                        <p className="text-[10px] text-gray-400">Alex Rivera</p>
                    </div>
                </div>
                    ...
                    {employeeNav.map((item) => {
                        ...
                        return (
                            <Link key={item.path} href={item.path} onClick={() => setSidebarOpen(false)}
                                className={`flex items-center gap-3 rounded-xl px-4 py-3 text-sm font-medium transition-all ${isActive ? "bg-violet-50 text-violet-700 dark:bg-violet-500/10 dark:text-violet-300" : "text-gray-600 hover:bg-gray-50 dark:text-gray-400 dark:hover:bg-gray-800"}`}>
            ...
            <div className="lg:ml-[260px]">
                <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-gray-200 bg-white/80 px-6 backdrop-blur-md dark:border-gray-800 dark:bg-gray-900/80">
                    ...
                    <div className="flex items-center gap-3">
                        <ThemeToggleButton />
                        <div className="flex items-center gap-2 rounded-full bg-violet-50 px-3 py-1.5 dark:bg-violet-500/10">
                            <div className="h-7 w-7 rounded-full bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center text-white text-xs font-bold">AR</div>
                            <span className="text-sm font-medium text-violet-700 dark:text-violet-300">Alex Rivera</span>
                        </div>
                    </div>
                </header>
